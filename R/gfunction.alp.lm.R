...
gfunction.alp.lm <- function(para, para.id, int){
  ...
  for(i in 1:nmodel){
    ...
    ga[, id.alp$start[i] - offset] <- (-1)
    
    k <- k + 1
    g.alp[[k]] <- ga
    rm(ga)
    
    id <- (id.alp$start[i]+1):id.alp$end[i]
    for(j in id){
      rx0 <- rx[, names(para)[j]]
      ga <- matrix(0, nrow = n, ncol = nt)
      ga[, id.alp$start[i] - offset] <- -2 * delta * rx0
      ga[, id - offset] <- -rx[, names(alp), drop = FALSE] * rx0
      ga[, (id.bet$start[i]):id.bet$end[i] - offset] <- -rx[, names(bet), drop = FALSE] * rx0
      k <- k + 1
      g.alp[[k]] <- ga
      rm(ga)
    }
