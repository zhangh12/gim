...
gfunction.bet.lo <- function(para, para.id, int){
  ...
  for(i in 1:nmodel){
    ...
    for(j in id){
      ...
      gb[, id.alp$start[i]:(id.alp$end[i]) - offset] <- -rx[, names(alp), drop = FALSE] * (rx0 * yi * (1-yi))
      gb[, id - offset] <- -rx[, names(bet), drop = FALSE] * (rx0 * yi * (1-yi))
      k <- k + 1
      g.bet[[k]] <- gb
      rm(gb)
    }
